#!/usr/bin/env node
/**
 * Script to import European election 2014 data (election metadata + datapoints) to MongoDB
 * Usage: node import_europeennes_2014.js
 */
...
async function importElectionMetadata() {
  ...
  const electionFilePath = path.join(__dirname, "europeennes_2014_election.json");
...
async function importDataPoints() {
  ...
  const datapointsFilePath = path.join(__dirname, "europeennes_2014_datapoints.json");
...
async function main() {
  console.log("=".repeat(80));
  console.log("🗳️  EUROPEAN ELECTION 2014 - COMPLETE IMPORT");
  console.log("=".repeat(80));
