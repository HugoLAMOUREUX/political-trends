#!/usr/bin/env node
/**
 * Script to import Legislative election 2012 data (election metadata + datapoints) to MongoDB
 * Usage: node import_legislatives_2012.js
 */
...
async function importElectionMetadata() {
  ...
  const electionFilePath = path.join(__dirname, "legislatives_2012_election.json");
...
async function importDataPoints() {
  ...
  const datapointsFilePath = path.join(__dirname, "legislatives_2012_datapoints.json");
...
async function main() {
  console.log("=".repeat(80));
  console.log("🗳️  LEGISLATIVE ELECTION 2012 - COMPLETE IMPORT");
  console.log("=".repeat(80));
