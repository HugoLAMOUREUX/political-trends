#!/usr/bin/env node
/**
 * Script to import Legislative election 2022 data (election metadata + datapoints) to MongoDB
 * Usage: node import_legislatives_2022.js
 */
...
async function importElectionMetadata() {
  ...
  const electionFilePath = path.join(__dirname, "legislatives_2022_election.json");
...
async function importDataPoints() {
  ...
  const datapointsFilePath = path.join(__dirname, "legislatives_2022_datapoints.json");
...
async function main() {
  console.log("=".repeat(80));
  console.log("🗳️  LEGISLATIVE ELECTION 2022 - COMPLETE IMPORT");
  console.log("=".repeat(80));
