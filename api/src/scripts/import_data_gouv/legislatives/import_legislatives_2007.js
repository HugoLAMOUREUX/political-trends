#!/usr/bin/env node
/**
 * Script to import Legislative election 2007 data (election metadata + datapoints) to MongoDB
 * Usage: node import_legislatives_2007.js
 */
...
async function importElectionMetadata() {
  ...
  const electionFilePath = path.join(__dirname, "legislatives_2007_election.json");
...
async function importDataPoints() {
  ...
  const datapointsFilePath = path.join(__dirname, "legislatives_2007_datapoints.json");
...
async function main() {
  console.log("=".repeat(80));
  console.log("🗳️  LEGISLATIVE ELECTION 2007 - COMPLETE IMPORT");
  console.log("=".repeat(80));
