#!/usr/bin/env node
/**
 * Script to import 2007 presidential election data from JSON to MongoDB
 * Usage: node import_presidential_2007.js
 */
...
async function importElectionMetadata() {
  ...
  const electionFilePath = path.join(__dirname, "presidential_2007_election.json");
...
async function importDataPoints() {
  ...
  const datapointsFilePath = path.join(__dirname, "presidential_2007_datapoints.json");
  ...
  const deleteResult = await DataPoint.deleteMany({
    election_id: { $in: ["presidentielle_2007_t1", "presidentielle_2007_t2"] },
  });
...
async function main() {
  console.log("=".repeat(80));
  console.log("🗳️  PRESIDENTIAL ELECTION 2007 - JSON IMPORT");
  console.log("=".repeat(80));
