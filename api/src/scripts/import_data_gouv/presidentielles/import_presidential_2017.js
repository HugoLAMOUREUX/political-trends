#!/usr/bin/env node
/**
 * Script to import 2017 presidential election data from JSON to MongoDB
 * Usage: node import_presidential_2017_from_json.js
 */
...
async function importElectionMetadata() {
  ...
  const electionFilePath = path.join(__dirname, "presidential_2017_election.json");
...
async function importDataPoints() {
  ...
  const datapointsFilePath = path.join(__dirname, "presidential_2017_datapoints.json");
...
async function main() {
  console.log("=".repeat(80));
  console.log("🗳️  PRESIDENTIAL ELECTION 2017 - JSON IMPORT");
  console.log("=".repeat(80));
